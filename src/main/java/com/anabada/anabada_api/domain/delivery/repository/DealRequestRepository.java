...
public interface DealRequestRepository extends JpaRepository<DealRequestVO, Long> {
    ...
    @Query(value = "select e from DealRequestVO e where ( e.responseItem= :selectedItem or e.requestItem =:selectedItem ) and e.state=:state order by  e.tradedAt desc ")
    public List<DealRequestVO> findByHistory(@Param("selectedItem") ItemVO selectedItem, @Param("state") int state);
}
